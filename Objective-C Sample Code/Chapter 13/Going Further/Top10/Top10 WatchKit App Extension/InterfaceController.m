...
- (void)awakeWithContext:(id)context
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(resetViewForSnapshot:) name:@"resetViewForSnapshot" object:nil];
}

#pragma mark - Snapshot 
- (void)resetViewForSnapshot:(NSNotification *)notification
{
...
- (void)presentAlert
{
    ...
    WKAlertAction *retryAction = [WKAlertAction actionWithTitle:@"Retry" style:WKAlertActionStyleDefault handler:^{
        [self reloadData];
    }];
    
    [self presentAlertControllerWithTitle:nil message:@"Top 10 list currently unavailable" preferredStyle:WKAlertControllerStyleSideBySideButtonsAlert actions:@[cancelAction, retryAction]];
}
