...
- (void)retrieveJSONFeedWithCompletionHandler:(void (^)(NSError *, NSArray *))completionHandler forNumberOfItems:(NSUInteger)numberOfSongs
{
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"https://itunes.apple.com/us/rss/topsongs/limit=%lu/json", (unsigned long)numberOfSongs]]];
...
- (JSONItem *)itemForDictionary:(NSDictionary *)dictionary
{
    ...
    NSArray *imagePreviewArray = dictionary[@"im:image"];
    item.imageURL = [NSURL URLWithString:[[imagePreviewArray lastObject] objectForKey:@"label"]];
    
    NSArray *audioPreviewArray = dictionary[@"link"];
    item.audioPreviewURL = [NSURL URLWithString:[[[audioPreviewArray lastObject] objectForKey:@"attributes"] objectForKey:@"href"]];
