...
class ComplicationController: NSObject, CLKComplicationDataSource {
    ...
    func getCurrentTimelineEntry(for complication: CLKComplication, withHandler handler: @escaping (CLKComplicationTimelineEntry?) -> Void) {
        ...
        if complication.family == .utilitarianLarge {
            let entry: CLKComplicationTimelineEntry = self.createUtilitarianLargeTimelineEntry(headerText: "Top 10", bodyText: self.timelineEntries.first!, date: Date())
            handler(entry)
        }
    }
    ...
    func getTimelineEntries(for complication: CLKComplication, after date: Date, limit: Int, withHandler handler: @escaping ([CLKComplicationTimelineEntry]?) -> Void) {
        ...
        for string: String in self.timelineEntries {
            ...
            if complication.family == .utilitarianLarge {
                timelineEntries.append(self.createUtilitarianLargeTimelineEntry(headerText: "Top 10", bodyText: string, date: dateOffset))
            }
    ...
    func getLocalizableSampleTemplate(for complication: CLKComplication, withHandler handler: @escaping (CLKComplicationTemplate?) -> Void) {
        ...
        if complication.family == .utilitarianLarge {
            let utilitarianLarge: CLKComplicationTemplateUtilitarianLargeFlat = CLKComplicationTemplateUtilitarianLargeFlat()
            utilitarianLarge.textProvider = CLKSimpleTextProvider(text: "Top Song: ---")
    ...
    func createUtilitarianLargeTimelineEntry(headerText: String, bodyText: String, date: Date) -> CLKComplicationTimelineEntry {
        
        let utilitarianLarge: CLKComplicationTemplateUtilitarianLargeFlat = CLKComplicationTemplateUtilitarianLargeFlat()
        utilitarianLarge.textProvider = CLKSimpleTextProvider(text: bodyText)
        
        return CLKComplicationTimelineEntry(date: date, complicationTemplate: utilitarianLarge)
    }
}
